...
parser.add_argument("--channels", type=int, default=1)  # 单通道
# parser.add_argument("--channels", type=int, default=3)  # 三通道
parser.add_argument("--dataset_name", type=str, default="mszs_train256_1")
...
vals_images = os.path.join(data_root, "val_image")
...
loss_dict = {"net_out": loss_func, "pid_out": loss_func}
model.compile(optimizer=Adam(lr=optimizer_name),
              loss=loss_dict,
              metrics=["accuracy", 'custom_precision', 'custom_recall', "f1_score"])

# metrics=["accuracy",'precision', 'recall', 'f1_score', 'auc', 'sensitivity', 'specificity',
#         'mean_iou', 'custom_precision', 'custom_recall',"cf1_score"])
# 自定义
# metrics=['accuracy', 'precision', 'recall', 'mis_alarm', 'false_alarm', 'iou_score', 'dice_score'])

print(get_flops(model))
model.summary()  # 输出模型各层的参数状d况

# 模型回调函数
# tensorboard --logdir=./weights/exp1/resunet/log
...
val_images = glob.glob(os.path.join(vals_images, "*.jpg")) + \
             glob.glob(os.path.join(vals_images, "*.png")) + \
             glob.glob(os.path.join(vals_images, "*.jpeg"))

num_train = len(images)
num_val = len(val_images)
print("train_count:", num_train)
print("val_count:", num_val)
...
data_gen_args = dict(rotation_range=0.2,
                     ...
                     fill_mode='nearest')
myGene = pidtrainGenerator(2, data_root,
                           'image', 'label', 'edge',
                           data_gen_args,
...
val_ge = pidtrainGenerator(2, data_root,
                           'val_image', 'val_label', 'val_edge',
                           data_gen_args,
                           image_color_mode=color_mode.get(channels),
                           save_to_dir=None,
                           target_size=target_size)
...
model.fit_generator(myGene, epochs=epochs,
                    ...
                    validation_steps=int(num_val / train_batch_size),
