...
# file:xor 
# author： jiangna
# desctription： 不存在边缘的指标计算,保存在文件中
...
"""
models = 'dv3+'
# tes = 'mszs_test256_1_4'
tes = 'archive_test256_1_4'

parser = argparse.ArgumentParser()
parser.add_argument("--channels", type=int, default=1)  # 默认单通道1 3
...
test_root = os.path.join("data", 'four', models, tes)

label_path = os.path.join(test_root, 'label')
images = glob.glob(os.path.join(label_path, "*.jpg")) + \
         glob.glob(os.path.join(label_path, "*.png"))

output = f'data/output/xor/{models}/{tes}'
if not os.path.exists(output):
    os.makedirs(output)


def adjustData(image, mask, plabel, flag_multi_class, num_class):
    assert mask.size != 0
    ...
    return image, mask, plabel


def TotalGenerator(batch_size, train_path, image_folder, mask_folder, plabel_folder, aug_dict,
    ...
    mask_generator = mask_datagen.flow_from_directory(
        ...
        classes=[mask_folder],
        class_mode=None,
        color_mode=mask_color_mode if channels != 3 else 'rgb',
        target_size=target_size,
        batch_size=batch_size,
        save_to_dir=save_to_dir,
    ...
    train_generator = zip(image_generator, mask_generator, plabel_generator)
    for (image, mask, plabel) in train_generator:
        image, mask, plabel = adjustData(image, mask, plabel, flag_multi_class, num_class)
        yield (image, mask, plabel)
...
for index in range(len(images)):
    image, y_true, y_pred = next(myGene)

    # y_pred = np.maximum(pre_y_label, pre_y_edge, out=None)
    inputss = np.hstack((image[0, :, :, :], y_true[0, :, :, :] * 255, y_pred[0, :, :, :] * 255))
    ...
    f1_list.append(f1)
    cucl = f"precision:{round(float(precision),4)}, recall:{round(float(recall),4)}, f1:{round(f1,4)}"
    print(cucl)

    font = cv2.FONT_HERSHEY_PLAIN
    # cv2.putText(inputss, cucl, (150, 240), font, 2, (128, 0, 128), 2)
    cv2.imwrite(f'{output}/{index}.png', inputss)
    write_file(f'{index}.png: {cucl}\n')
