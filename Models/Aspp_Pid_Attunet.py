...
def ASPP_PID_AttUNet(n_classes=2, channels=1, input_height=256, input_width=256):
    ...
    outs = Conv2D(64, 2, padding='same')(d2)
    outs = Conv2D(1, 1, padding='same')(outs)
    pid_out = PidiNet(outs)

    out = Conv2D(64, 2, padding='same', activation='sigmoid')(d2)
    out = Conv2D(2, 2, padding='same')(out)
    net_out = Conv2D(1, 1, padding='same', activation='sigmoid', name="net_out")(out)  # 添加

    # todo
    asb = pid_out + net_out
    model = Model(input=inputs, output=[net_out, pid_out, asb])
    return model
